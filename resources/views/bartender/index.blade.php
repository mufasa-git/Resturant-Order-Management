...
@extends(Auth::user()->user_type == 'Admin' ? 'layouts.app' : 'layouts.bartender_app')
...
<section class="vbox">
    <section class="scrollable padder">
        ...
        <div class="m-b-md">
            <h3 class="m-b-none">Bartender</h3>
        </div>
                                ...
                                <ul class="price">
                                    ...
                                    @foreach($order_lists as $order_list)
                                        @if($order_list->order_info_id == $order_info->id)
                                            @if($order_list->product->category == 'Drink')
...
$(document).ready(function(){
  ...
  setThumbsFunc=(id)=> {
    ...
    $.ajax({
    url: "{{ route('order_info.bartender_thumbs') }}",
    method: "POST",
    data: {
        _token: '{!! csrf_token() !!}',
        tableid: id,
  ...
  setCheckFunc=(id)=> {
    ...
    $.ajax({
    url: "{{ route('order_info.bartender_check') }}",
    method: "POST",
    data: {
        _token: '{!! csrf_token() !!}',
        tableid: id,
